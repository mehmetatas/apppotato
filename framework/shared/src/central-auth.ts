...
export const centralVerifyAuth = api("POST", "/v1/auth/verify")
  .s2s()
  .withRequest(centralVerifyAuthRequest)
  .withResponse(centralVerifyAuthResponse);

// POST /v1/auth/email — S2S: send magic link email
export const centralSendEmail = api("POST", "/v1/auth/email")
  .s2s()
  .withRequest(centralSendEmailRequest)
  .withResponse(centralSendEmailResponse);
